...
```julia
# Define a Ball at [0,0] with radius 1
center = [0, 0]
radius = 1
...
# Check containment
println(isContained(ball, pt_inside)) # true (distance is sqrt(2)/4 ≈ 0.353 < 1)
println(isContained(ball, pt_outside)) # false (distance is 3/sqrt(2) ≈ 2.121 > 1)
println(isContained(ball, pt_boundary)) # true (distance is 1 = 1)
```
